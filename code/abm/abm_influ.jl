...
function cpr_abm(
  ...
  sinf = zeros(nrounds, ngroups, nsim)
  ...
  for sim = 1:nsim
    ...
    for t ∈ 1:nrounds
      ...
      policyfine1 = zeros(ngroups)
      policyfine2 = zeros(ngroups)

      if power == false
         wei = rand(Normal(.5, .0),n)
       end
       if power == "wealth" t ==1 ? wei = rand(Normal(.5, .01),n) : wei = copy(payoff) end
       if power == "poor" t ==1 ? wei = rand(Normal(.5, .01),n) : wei = abs.(payoff.-findmax(payoff)[1]) end
       if power == "income" t ==1 ? wei = rand(Normal(.5, .01),n) : wei = copy(payoff_round).+1 end
       if power == "influence" t ==1 ? wei = rand(Normal(.5, .01),n) : wei = influence end
       wei = AnalyticWeights(wei)
       for i in 1:ngroups
           if group_status[i] == 1
              policy[i]=median(harv_limit[gid.==i], wei[gid.==i])
              policyfine1[i]=median(fines1[gid.==i], wei[gid.==i])
              policyfine2[i]=median(fines2[gid.==i], wei[gid.==i])
            end
       end
      ...
      #Calcualte pollition costs
       ...
       if pollution > 0
        ...
        if fine_start != nothing
           for i = 1:ngroups
             if sum(loc.==i) != 0
               fine1total[loc .==i] = caught[loc.==i] .* policyfine1[i]
               fine2total[gid .==i] = caught2[gid.==i] .* policyfine2[i]
           end
         end
       end

        fc1 = fine1total.*ifelse.(isnan.(effort./eg2[loc]), 0,
           ((effort./eg2[loc]) .* X[loc]) .* price)

        fc2 = fine2total.*ifelse.(isnan.(effort./eg2[loc]), 0,
          ((effort./eg2[loc]) .* X[loc]) .* price)
    ...
    ############ In a world without boundaries .... ##################
      ...
      if social_learning == true
        ...
        trans_error = rand(n,9) .< fidelity
        n_error = [[rand(Normal(), n)] [rand(Normal(1, .02), n)] [rand(Normal(), n)] [rand(Normal(), n)] [rand(Normal(1, .02), n)] [rand(Normal(1, .02), n)] [rand(Normal(), n)] [rand(Normal(), n)]]
        ...
        og_type = ifelse.(trans_error[:,8], inv_logit.(logit.(og_type[models]) .+ n_error[7]), og_type[models])
        influence = ifelse.(trans_error[:,9], inv_logit.(logit.(influence[models]) .+ n_error[8]), influence[models])
      ...
      sinf[year,:,sim] .= round.(reportSkew(influence, gid, ngroups), digits=4)
