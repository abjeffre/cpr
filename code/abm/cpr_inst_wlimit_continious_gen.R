...
cpr_institution_abm <- function(
  ...
  marginal = FALSE,             # This controls whether payoffs have are marginally decreasing in value
  ...
  for (sim in 1:nsim){
    ...
    for(t in 1:nrounds){
      ...
      if(inst == TRUE){
        ...
        caught <- rep(0, n)
        for(i in 1:n){
          prob_caught <- pbeta(sum(punish_type[gid == loc[i]])/(n/ngroups), 1, monitor_tech)
          if(loc[i] != gid[i]){
