...
function cpr_abm(
  ...
  wages = .125,                   # Wage rate in other sectors - opportunity costs
  ...
  market_absorbtion = nothing,
  marketPrices = false         #THIS IS HOW WELL THE MARKET CAN ABSORB LABOR uses BETA CDF
)


  ################################################
  ########### BASIC CHECKS #######################
    price_init = copy(price)
    agPrice_init = copy(agPrice)
...
#   )
  ...
  for sim = 1:nsim
      seed = seed + sim
      price = copy(price_init)
      agPrice = copy(agPrice_init)
    ...
    for t ∈ 1:nrounds
      ...
      for i = 1:length(experiment_group)
        ...
        if experiment_effort != 0
            ...
            effort[agents.gid.==experiment_group[i], [1, 3]] .= (1-experiment_effort)/(nsectors-1)  
        end
      end
      ...
      if catch_before == true
        ...
        if harvest_type == "collective"
         ...
         if marketPrices == true
             price = (1-sum(HG)/sum(GetGroupHarvest(ones(n), loc, kmax, kmax, tech, labor, degrade, ngroups)))*price_init
         end
        else
          HG=GetHarvest(temp_effort, loc, K, kmax, tech, labor, degrade, necessity, ngroups)
          GH=reportSum(HG, loc, ngroups)
          if marketPrices == true
                price = (1-sum(HG)/sum(GetHarvest(ones(n), loc, kmax, kmax, tech, labor, degrade, necessity, ngroups)))*price_init
          end
        end
      else
        if harvest_type == "collective"
      ...
      WL = current_wages*(effort[:,1].*100)*tech
    
      #Agricultural Production
      if agSector == true
          AG = GetAgHarvest(effort[:,3], agents.gid, K, kmax, tech, labor, agDegrade, necessity, ngroups)
          if marketPrices == true
            agPrice = agPrice_init * (1-(sum(AG)/sum(GetAgHarvest(ones(n), agents.gid, ones(ngroups), kmax, tech, labor, agDegrade, necessity, ngroups))))
          end
          AG=AG.*agPrice
      else
         AG = zeros(n)
      ...
      #     GetAgHarvest2(effort[:,3], agents.gid, K, kmax, tech, labor, agDegrade, necessity, ngroups)
      ...
      ############# RECORD HISTORY ####################
        ...
        history[:agEffort][year,:,sim] .=convert.(Float16,report(effort[:,3], agents.gid, ngroups))
        history[:wEffort][year,:,sim] .=convert.(Float16,report(effort[:,1], agents.gid, ngroups))
        if rec_history == true 
