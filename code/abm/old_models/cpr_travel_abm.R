...
cpr_travel_abm <- function(
  ...
  for (sim in 1:nsim){
    ...
    harvest <- rep(NA, ngen)
    wage <- rep(NA, ngen)
    effort_h <- rep(NA, ngen)
    payoff_h <- rep(NA, ngen)
    id <- 1:n 
    ...
    for(g in 1:ngen){
      ...
      for(t in 1:nrounds){
        ...
        for (i in 1:n){
          ...
          # and harvest
          # note that we have an exponential travel cost that the costs are squared
        proposal <- which(rmultinom(1, 1,  softmax(-(distances[,gid[i]]^2))) ==1)
        loc[i] <- ifelse(K[proposal] > K[gid[i]], proposal, gid[i])
        }
