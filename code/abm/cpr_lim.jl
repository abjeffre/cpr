function cpr_abm(
  ...
  defensibility = 1,          # This sets the minmum number of people needed
  punish_cost = 0.03,        # This is the cost that must be paid for individuals <0 to monitor their forests - For the default conditions this is about 10 percent of mean payoffs
  ...
  distance_adj =0.9,
  travel_cost = .03,           # This basically controls the travel time for individuals who travel to neightboring communities to steal from Note that higher values mean less leakage
  ...
  fidelity = 0.05,            # This is the fidelity of social transmission
    ...
    for t ∈ 1:nrounds
      ...
      for i = 1:length(experiment_group)
        ...
        if experiment_punish != 0
          punish_type[gid .==experiment_group[i]] = rbinom(sum(gid.==experiment_group[i]),1, experiment_punish)
        end
      ...
      if inst == true
        ...
        for i = 1:n
          ...
          if loc[i] != gid[i]
            ...
            if self_policing ==true
              if hg_temp[i] >=  median(harv_limit[gid[asInt(loc[i])]]) caught[i] = rbinom(1, 1, prob_caught)[1]
              end
            end
          end
        end
        ...
        for i = 1:ngroups
          temp = (X_siezed[i]/sum(punish_type[gid.==i])).*price
          if isinf(temp) | isnan(temp)
            sg[i] = 0
          else
            sg[i] = temp
        ...
        payoff_round = hg.*(1 .- caught) + wl + sg[gid].*punish_type + fp[gid].*punish_type - mc - fc.*caught - travel_cost.*leakage_type
