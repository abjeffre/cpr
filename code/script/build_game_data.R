...
init <- function(x){
  df <- as.data.frame(matrix(NA, nrow = x, ncol = 24))
  names(df) <- c("id", "ward", "twealth",  "game_id", "rounds_p", "fcover", "lcover", "pop", "prob", "over", "local", "punished", "buis", "gov", "out", "pemba", "neigh", "harvest_p", "final_forest_p", "harvest_np", "final_forest_np", "gender", "age", "seen")
  return(df)
}
