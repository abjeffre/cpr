...
@JLD2.load("C:\\Users\\jeffr\\Documents\\work\\cpr\\data\\abm\\abm_dat_pun_hm_leakp19.jld2")

###LOW LABOR ####
p_arr1 = Plots.Plot{Plots.GRBackend}[]
resize!(p_arr1, 9)
p_arr=reshape(p_arr1, 3, 3)
...
for j in 1:9
    ...
    m = zeros(20,20)
    for i = 1:length(high)
        m[i]  = mean(mean(high[i]["effort"][:,j,:], dims =2) - mean(low[i]["effort"][:,j,:], dims = 2))
        end
    using Plots
    gr()
     p_arr[j]=p1=heatmap(m,
        c=:thermal, clim= (-.5, .5), xlab = "", ylab= "",
...
plot(p_arr1[1],  p_arr1[2], p_arr1[3], p_arr1[4], p_arr1[5],
p_arr1[6], p_arr1[7], p_arr1[8], p_arr1[9], layout = (3,3))
