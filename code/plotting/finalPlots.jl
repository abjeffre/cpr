...
labor = unique(S[:,6])
sort!(travelcost)
maintit = ["low" "low" "low" "med" "med" "med" "med" "high" "high" "high"]
                ...
                for j in 1:length(travelcost)
                        ...
                        gr()
                         p_arr[j,k,r]=p1=heatmap(m,
                             ...
                             title = ifelse(r==1, string("Travel Cost: ", maintit[j]), " "),
                             titlefontsize = 9)
                end
        end
end
...
maintit = ["low" "low" "low" "med" "med" "med" "high" "high" "high" "high"]
for r in 1:length(pars)
        for k in 1:length(labor)
                for j in 1:length(punishcost)
                        ...
                        gr()
                         p_arr[j,k,r]=p1=heatmap(m,
                             ...
                             xticks = ([2,19], ifelse(r == 2, ("Low", "High"), (" ", " "))),
                             yticks = ([2,19], ifelse(k == 1, ("Low", "High"), (" ", " "))),
                             title = ifelse(r==1, string("Punish Cost: ", maintit[j]), " "),
                             titlefontsize = 9)
                end
        end
end
...
pars = [:effort, :punish2, :limit]
m_arr = []
resize!(m_arr, length(punishcost)*length(limit)*length(pars))
m_arr=reshape(m_arr, length(punishcost), length(limit), length(pars))
...
for r in 1:length(pars)
 ...
 @JLD2.save("m_arr_p2.jld2", m_arr, S)


 # on local machine
 @JLD2.load("cpr\\data\\abm\\m_arr_p2.jld2")
...
ps1 = [p_arr[1, 1, 1] p_arr[4, 1, 1] p_arr[6, 1, 1]]
ps2 = [p_arr[1, 1, 2] p_arr[4, 1, 2] p_arr[6, 1, 2]]
ps3 = [p_arr[1, 1, 3] p_arr[4, 1, 3] p_arr[6, 1, 3]]
...
###############################################################
########################## COVARIANCE ##########################
@JLD2.load("cpr\\data\\abm\\m_arr_cov.jld2")
...
@JLD2.save("m_arr_cov.jld2", m_arr)
        ...
        println(r)
            for k in 1:length(labor)
                for j in 1:length(punishcost)
                        ...
                        c=cols[r],
                        clim= ifelse(r==3, (-2,2), (-1,1)),
                        xlab = ifelse(r==1, "Wages", " "),
                        ylab = ifelse(j==1, "Price", " "),
                        yguidefontsize=9,
                        xguidefontsize=9,
                        legend = false,
                        xticks = ([2,19], ifelse(r == 1, ("Low", "High"), (" ", " "))),
                        yticks = ([2,19], ifelse(k == 1, ("Low", "High"), (" ", " "))),
                        title = ifelse(r==1, string("Punish Cost: ", maintit[j]), " "),
                        titlefontsize = 9)
                end
                ...
                for j in 1:length(punishcost)
                        ...
                        gr()
                         p_arr[j,k,r]=p1=heatmap(m,
                             ...
                             title = string("Pun cost = :", maintit[j]), titlefontsize = 9)
                end
        end
end
...
png("cpr\\output\\cov.png")
 ...
 # or @JLD2.load("abmDatFullNoSharing.jld2")
 #@JLD2.load("abmDatFullSmallLearning.jld2")
 #@JLD2.load("abmDatFullSmallFine.jld2")

 #@JLD2.load("abmDatP1P2.jld2")
 punishcost = unique(S[:,8])
 sort!(punishcost)
 limit = unique(S[:,5])
 sort!(punishcost)
     ...
     println(r)
         for k in 1:length(limit)
                 for j in 1:length(punishcost)
                         ...
                         m = zeros(20,20)
                         for i = 1:length(high)
                             m[i]  = mean(mean(low[i][pars[r]][300:end,2:end,:], dims =2)) # mean(mean(high[i][pars[r]][300:end,2:end,:], dims =2)) -mean(low[i][pars[r]][300:end,2:end,:], dims =2))# - mean(low[i]["effort"][:,2,:], dims = 2))
                        end
                          m_arr[j,k,r]=m
                 end
         end
 ...
 @JLD2.save("m_arr_pemba.jld2", m_arr, S)

###### On Local Machine 

 @JLD2.load("cpr\\data\\abm\\m_arr_new.jld2")
 @JLD2.load("cpr\\data\\abm\\m_arr_full.jld2")
 @JLD2.load("cpr\\data\\abm\\m_arr_p2.jld2")
 @JLD2.load("cpr\\data\\abm\\m_arr_p1p2.jld2")
 @JLD2.load("cpr\\data\\abm\\m_arr_pemba.jld2")
 ...
 p_arr = Plots.Plot{Plots.GRBackend}[]
 resize!(p_arr, length(punishcost)*length(limit)*length(pars))
 p_arr=reshape(p_arr, length(punishcost), length(limit), length(pars))
 cols = [:RdBu_9, cgrad(:roma, rev = false),  :lighttemperaturemap, cgrad(:PuOr_9, rev = false), :PiYG_9]
...
j = 1
 ...
 ps1 = [p_arr[j, i, 1] p_arr[4, 1, 1] p_arr[6, 1, 1]]
 ps2 = [p_arr[j, i, 2] p_arr[4, 1, 2] p_arr[6, 1, 2]]
 ps3 = [p_arr[j, i, 3] p_arr[4, 1, 3] p_arr[6, 1, 3]]
 ps4 = [p_arr[j, i, 4] p_arr[4, 1, 4] p_arr[6, 1, 4]]
 ps5 = [p_arr[j, i, 5] p_arr[4, 1, 5] p_arr[6, 1, 5]]
...
png("cpr\\output\\full.png")


 
###################################################################
################## MAKE A BRAND NEW SET HERE FOR PLOTTING #########


ps1 = [p_arr[j, i, 1] p_arr[1, i, 1] p_arr[1, i, 1]]
ps2 = [p_arr[j, i, 2] p_arr[1, i, 2] p_arr[1, i, 2]]
ps3 = [p_arr[j, i, 3] p_arr[1, i, 3] p_arr[1, i, 3]]
ps4 = [p_arr[j, i, 4] p_arr[1, i, 4] p_arr[1, i, 4]]
ps5 = [p_arr[j, i, 5] p_arr[1, i, 5] p_arr[1, i, 5]]
