...
model{
     ...
     a[2] ~ normal(4, 1);
     for(i in 1:N){
        mu[i] = a[1] + bX[1][X1[i]]*sigma_X1[1]  + bX2[1]*sum(delta_j[1][1:X2[i]]) + bS[1][S[i]]*sigma_S[1] + bINT[1][INT[i]]*sigma_INT[1] +   bC[1][C[i]]*sigma_C[1]  + Z[i]*beta[1];
        p[i] =  a[2] + bX[2][X1[i]]*sigma_X1[2]  + bX2[2]*sum(delta_j[2][1:X2[i]]) + bS[2][S[i]]*sigma_S[2] + bINT[2][INT[i]]*sigma_INT[2] +   bC[2][C[i]]*sigma_C[2] +  Z[i]*beta[2];
      }
    // Hurdle Stage 1
